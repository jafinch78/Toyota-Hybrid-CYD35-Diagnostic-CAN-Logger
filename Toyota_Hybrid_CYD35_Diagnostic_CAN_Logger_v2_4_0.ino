/*
  Toyota Hybrid CYD 3.5-inch Diagnostic CAN Logger v2.4.0
...
void setup() {
  ...
  delay(100);
  Serial.println("# ToyotaHybridCAN Diagnostic Logger v2.4.0");
...
void writeReadme() {
  ...
  if (!file) return;
  file.println("ToyotaHybridCAN Capture Package v1.4 / firmware v2.4.0");
...
void writeManifest(bool closedCleanly) {
  ...
  file.println("  \"firmware_version\": \"2.4.0\",");
...
void updateDisplay() {
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  if (displayPage == 0) {
    tft.drawString("TOYOTA HYBRID CAN LOGGER v2.4", 10, 8, 2);
    ...
    String twaiText = twaiNormalMode ? "NORMAL 500k DIAGNOSTIC" : "LISTEN-ONLY 500k";
    if (bleConnected) twaiText += " BLE";
