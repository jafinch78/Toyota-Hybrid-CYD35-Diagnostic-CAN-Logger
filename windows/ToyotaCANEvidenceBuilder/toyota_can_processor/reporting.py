...
def write_offline_report(path: Path, summary: dict[str, Any], session_dir: Path,
    ...
    counters = summary.get("counter_reconciliation", {})
    ...
    outliers = _csv(session_dir / "CAN_OCR_OUTLIERS.csv", 500)
    rejected = _csv(session_dir / "OCR_REJECTED_SEMANTICS.csv", 500)
    decisions = _csv(session_dir / "DATABASE_FIELD_DECISIONS.csv", 500)
...
<div>Session {html.escape(str(summary.get('session', '')))} · Evidence Builder {__version__} · Database {html.escape(str(database.get('version', '')))} schema {html.escape(str(database.get('schema_version', '')))}</div></header>
...
<div class='card'><div>Label-based CAN/OCR pairs</div><div class='value'>{grading.get('label_based_can_ocr_pairs', grading.get('can_ocr_pairs', 0))}</div></div>
<div class='card'><div>Graph CAN matches</div><div class='value'>{grading.get('graph_can_matches', 0)}</div></div>
<div class='card'><div>Effective evidence events</div><div class='value'>{grading.get('effective_evidence_events', 0)}</div></div>
<div class='card'><div>Unique OCR frames</div><div class='value'>{grading.get('unique_ocr_frames', 0)}</div></div>
<div class='card'><div>Counter reconciliation</div><div class='value'>{html.escape(str(counters.get('status', '—')))}</div></div>
<div class='card'><div>BLE RMS</div><div class='value'>{alignment.get('residual_rms_ms', '—')} ms</div></div></div>
<section class='panel'><h2>Decoder database provenance</h2><table><tbody>
<tr><th>File</th><td>{html.escape(str(database.get('path', '')))}</td></tr>
<tr><th>Version / schema</th><td>{html.escape(str(database.get('version', '')))} / {html.escape(str(database.get('schema_version', '')))}</td></tr>
<tr><th>SHA-256</th><td><code>{html.escape(str(database.get('sha256', '')))}</code></td></tr>
<tr><th>Definitions</th><td>{database.get('definition_count', 0)} ({database.get('field_specific_definition_count', 0)} field-specific)</td></tr>
<tr><th>Candidate registry</th><td>{database.get('candidate_observation_count', 0)} observations; metadata-only decoding exclusion: {html.escape(str(database.get('candidate_registry_ignored_for_decoding', False)))}</td></tr>
<tr><th>Unchanged after processing</th><td>{html.escape(str(database.get('verified_unchanged_after_processing', False)))}</td></tr>
</tbody></table></section>
...
<section class='panel'><h2>Local evidence grading</h2>
<p>Matching: {html.escape(str(grading.get('matching_method', '')))}. Zero-pair diagnosis: {html.escape(str(grading.get('zero_pair_diagnosis', '')))}.</p>
{_table(grades, ['DecoderKey','Field','ArrayIndex','DecodedSamples','OCRPairs','UniqueOCRFrames','EffectiveIndependentSamples','AgreementRate','RMSE','MedianAbsoluteError','TimeSpan_s','DynamicRange','BoundsFailures','UnitCheckFailures','DatabaseEvidenceGrade','PreliminaryLocalGrade','DecisionReason'], 500)}</section>
<section class='panel'><h2>CAN/OCR outliers</h2>{_table(outliers, ['DecoderKey','Field','ArrayIndex','OCRFrame','CANValue','OCRValue','AbsoluteError','Tolerance','DetectedApp','DetectedLayout'], 500)}</section>
<section class='panel'><h2>Rejected OCR semantics</h2>{_table(rejected, ['DecoderKey','Field','ArrayIndex','OCRFrame','OCRLabel','Unit','DetectedApp','DetectedLayout','Reason'], 500)}</section>
<section class='panel'><h2>Field-level database decisions</h2>{_table(decisions, ['DecoderKey','Field','ArrayIndex','DatabaseEvidenceGrade','PreliminaryLocalGrade','DecisionReason'], 500)}</section>
<section class='panel'><h2>Counter reconciliation</h2><table><tbody>
<tr><th>Status</th><td>{html.escape(str(counters.get('status', '')))}</td></tr>
<tr><th>Raw / processed</th><td>{counters.get('raw_records_parsed', '')} / {counters.get('frames_processed_by_logger', '')}</td></tr>
<tr><th>Session received / since boot</th><td>{counters.get('session_received_frames', '')} / {counters.get('received_frames_since_boot', '')}</td></tr>
<tr><th>Queue / SD loss indicators</th><td>{html.escape(str(counters.get('loss_indicators_present', '')))}</td></tr>
</tbody></table></section>
