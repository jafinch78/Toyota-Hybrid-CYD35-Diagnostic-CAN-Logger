...
class Application(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Toyota CAN Evidence Builder 1.0.4")
    ...
    def _build(self) -> None:
        ...
        self._picker(frame, "Decoder database JSON (blank = bundled v0.5.9)", self.database, self._choose_database)
    ...
    def _choose_database(self) -> None:
        selected = filedialog.askopenfilename(
            filetypes=[("Toyota CAN database", "*.json"), ("All files", "*.*")])
        if selected: self.database.set(selected)
    ...
    def _start(self) -> None:
        ...
        options = ProcessingOptions(
            ...
            database_path=Path(self.database.get()) if self.database.get() else None)
