...
def _ocr_tokens(video: Path, timestamps: list[float], directory: Path,
                prefix: str, ffmpeg: str, tesseract: str) -> list[set[str]]:
    results = []
    for index, timestamp in enumerate(timestamps):
        frame = directory / f"{prefix}_{index:02d}.png"
        subprocess.run([
            ffmpeg, "-hide_banner", "-loglevel", "error", "-y", "-ss", f"{timestamp:.3f}",
            "-i", str(video), "-frames:v", "1", str(frame),
        ], check=True, **hidden_process_kwargs())
        result = subprocess.run(
            [tesseract, str(frame), "stdout", "--psm", "11"], check=False,
            capture_output=True, text=True, encoding="utf-8", errors="replace",
            **hidden_process_kwargs())
        results.append({token.lower() for token in re.findall(r"[A-Za-z0-9_.%+-]{2,}", result.stdout)})
    return results


def compare_proxy_ocr(original: Path, proxy: Path, duration_seconds: float) -> dict[str, Any]:
    """Compare OCR tokens at identical timestamps before approving a proxy."""
    ffmpeg = resolve_tool("ffmpeg")
    tesseract = resolve_tool("tesseract")
    if not ffmpeg or not tesseract:
        return {"approved_for_ocr": False,
                "decision": "MATCHED_FRAME_TEST_UNAVAILABLE",
                "reason": "FFmpeg and Tesseract are required for proxy approval"}
    if duration_seconds <= 0:
        return {"approved_for_ocr": False, "decision": "INVALID_DURATION"}
    timestamps = [duration_seconds * fraction for fraction in (0.10, 0.30, 0.50, 0.70, 0.90)]
    with tempfile.TemporaryDirectory(prefix="toyota_proxy_compare_") as temporary:
        root = Path(temporary)
        original_tokens = _ocr_tokens(original, timestamps, root, "original", ffmpeg, tesseract)
        proxy_tokens = _ocr_tokens(proxy, timestamps, root, "proxy", ffmpeg, tesseract)
    comparisons = []
    for timestamp, source, derivative in zip(timestamps, original_tokens, proxy_tokens):
        recall = len(source & derivative) / len(source) if source else (1.0 if not derivative else 0.0)
        comparisons.append({
            "timestamp_seconds": round(timestamp, 3), "original_token_count": len(source),
            "proxy_token_count": len(derivative), "original_token_recall": round(recall, 6),
        })
    recalls = [row["original_token_recall"] for row in comparisons]
    median_recall = statistics.median(recalls)
    approval = median_recall >= 0.80 and min(recalls) >= 0.50
    return {
        "approved_for_ocr": approval,
        "decision": "APPROVED" if approval else "REJECTED",
        "method": "TESSERACT_MATCHED_TIMESTAMPS_TOKEN_RECALL",
        "median_original_token_recall": round(median_recall, 6),
        "minimum_original_token_recall": round(min(recalls), 6),
        "frames": comparisons,
    }
...
def assess_or_create_review_proxy(video: Path, output_root: Path,
    ...
    if already_compact:
        ...
        if not ffmpeg:
            ...
            report["ocr_proxy_validation"] = compare_proxy_ocr(
                video, target, original["duration_seconds"])
            report["approved_for_ocr"] = bool(
                report["ocr_proxy_validation"].get("approved_for_ocr"))
