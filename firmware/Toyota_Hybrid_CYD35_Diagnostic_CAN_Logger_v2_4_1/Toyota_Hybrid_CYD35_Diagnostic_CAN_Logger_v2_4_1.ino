/*
  Toyota Hybrid CYD 3.5-inch Diagnostic CAN Logger v2.4.1
...
void setup() {
  ...
  delay(100);
  Serial.println("# ToyotaHybridCAN Diagnostic Logger v2.4.1");
  ...
  if (canQueue == nullptr || diagnosticQueue == nullptr ||
      bleSyncQueue == nullptr || bleCommandQueue == nullptr) {
...
bool initializeSD() {
  ...
  // session files and opening metadata files. Reserve explicit headroom.
...
void writeReadme() {
  ...
  if (!file) return;
  file.println("ToyotaHybridCAN Capture Package v1.4 / firmware v2.4.1");
...
void writeManifest(bool closedCleanly) {
  ...
  file.println("  \"firmware_version\": \"2.4.1\",");
...
void initializeBleSync() {
  ...
  char deviceName[24];
  snprintf(deviceName, sizeof(deviceName), "ToyotaCYD-%04X", suffix);
  BLEDevice::init(deviceName);
  bleServer = BLEDevice::createServer();
  if (bleServer == nullptr) {
    bleLinkState = BLE_LINK_ERROR;
    Serial.println("# BLE ERROR,CREATE_SERVER");
    return;
  }
  ...
  if (service == nullptr) {
    bleLinkState = BLE_LINK_ERROR;
    Serial.println("# BLE ERROR,CREATE_SERVICE");
    return;
  }
  ...
  if (command == nullptr) {
    bleLinkState = BLE_LINK_ERROR;
    Serial.println("# BLE ERROR,CREATE_COMMAND_CHARACTERISTIC");
    return;
  }
  ...
  if (bleResponseCharacteristic == nullptr) {
    bleLinkState = BLE_LINK_ERROR;
    Serial.println("# BLE ERROR,CREATE_RESPONSE_CHARACTERISTIC");
    return;
  }
  ...
  if (advertising == nullptr) {
    bleLinkState = BLE_LINK_ERROR;
    Serial.println("# BLE ERROR,CREATE_ADVERTISING");
    return;
  }
...
void updateDisplay() {
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  if (displayPage == 0) {
    tft.drawString("TOYOTA HYBRID CAN LOGGER v2.4.1", 10, 8, 2);
